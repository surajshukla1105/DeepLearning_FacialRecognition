...
vAR_op = configParser.get('My Section', 'op')
...
test_data_dir = configParser.get('My Section', 'vValidationDataDirectory')
...
def gET_ImagesAndLabels():
    ...
    for filename in filelist:
        ...
        for (x,y,w,h) in faces:
            ...
            Emot = list(filter(lambda x: x.isalpha(), ID))
            Emot = ''.join(Emot)
            emotion.append(Emot)
            IDs.append(Id)
    return faceSamples, emotion,IDs

faces, emotion, FileNames = gET_ImagesAndLabels()
facesI = np.array(faces)

vAR_dim = (150,150)
i=0
resized = facesI
os.chdir(vAR_PathResized)

### Save image for faces as processed training set
for i in range(0,len(faces)):
    # perform the actual resizing of the image and show it
    resized[i] = cv2.resize(facesI[i], vAR_dim, interpolation = cv2.INTER_AREA)
    Id=FileNames[i]
    cv2.imwrite(os.path.join(emotion[i],str(Id.split('\\')[-1])) ,resized[i])
...
resized[9].shape
faces[0].shape
nb_train_samples = len(faces)
############ For Validation Set #####################################

filelist = glob.glob(vAR_ReadValidationImages)
...
os.chdir(vAR_ValidationDataDirectory)

## Saving processed images of faces as validation set
for i in range(0,len(faces)):
    # perform the actual resizing of the image and show it
    resized[i] = cv2.resize(facesI[i], vAR_dim, interpolation = cv2.INTER_AREA)
    Id=FileNames[i]
...
#resized = facesI
#
#os.chdir('D:\\1 DataScience Code and Data\\Gender classification\\FinalUnseenTest')
#os.chdir('D:\\1 DataScience Code and Data\\Gender classification\\FinalUnseenTest\\Out')
### Saving processed images of faces as validation set
#for i in range(0,len(faces)):
#    # perform the actual resizing of the image and show it
#    resized[i] = cv2.resize(facesI[i], vAR_dim, interpolation = cv2.INTER_AREA)
#    Id=FileNames[i]
#    cv2.imwrite(os.path.join(gender[i],str(Id.split('\\')[-1])) ,resized[i])
#resized[i].shape 
#os.chdir(vAR_PathPrj)
################### Train CNN ######
#######################################
...
bottleneck_features_TEst = model_vgg.predict_generator(test_generator_bottleneck, 55)
np.save(open('bottleneck_features_TEst.npy', 'wb'), bottleneck_features_TEst)


train_data = np.load(open('bottleneck_features_train.npy', 'rb'))
train_labels = np.array(train_generator_bottleneck.classes)

validation_data = np.load(open('bottleneck_features_validation.npy', 'rb'))
validation_labels = np.array(validation_generator_bottleneck.classes)

Test_data = np.load(open('bottleneck_features_TEst.npy', 'rb'))
Test_labels = np.array(test_generator_bottleneck.classes)
...
## Save weight of model for future use
model_top.save_weights('bottleneck_40_epochs.h5')


## Evaluating the model on unseen data
vAR_Evaluate  = model_top.evaluate(Test_data, Test_labels)
    ...
    ## Converted image to Grayscale for analysis
        ...
        if len(facerect) > 0:
            ...
            for (x,y,w,h) in facerect:
                ...
                Emt = Emotion[int(result)]
                ## Put text on screen
                #cv2.putText(im,Emt, (x,y), cv2.FONT_HERSHEY_SIMPLEX, 2, color)
                pat = str(i)+'.jpg'
                PathImg = os.path.join(vAR_PathForTableau, str(pat))
                pos_frame = cam.get(cv2.CAP_PROP_POS_FRAMES)
                ## Save image to file system
                cv2.imwrite(PathImg,image)
                
                ## Save data to CSV
                RowData = Emt,PathImg,i,vAR_Evaluate[1]
                i = i+1
                c.writerow(RowData)
                #cv2.imshow('im',im) 
